...
/**
 * Premium airline autocomplete input (Kayak-style).
 *
 * Props:
 ...
 */
export default function AirlineAutocomplete({ placeholder = 'Airline name or code', value, onChange }) {
  ...
  const handleInput = (e) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const data = await searchAirlines(text);
  ...
  const handleSelect = (item) => {
    setInputText(`${item.name} (${item.iata})`);
    setResults([]);
    setOpen(false);
    onChange(item.iata);
  };
      ...
      {showDropdown && (
        <div style={styles.dropdown}>
          ...
          {!loading && results.length === 0 && inputText.length >= 2 && (
            <div style={styles.hint}>No airlines found</div>
          )}
