import time, requests, logging, random
...
def generate_mock_fleet():
    return [{
        "hex_id": f"SIM{random.randint(100,999)}",
        "flight_number": f"[SIMULATED] N{random.randint(1000,9999)}",
...
def fetch_radar():
    try:
        ...
        if not payload: raise ValueError("No planes.")
    except Exception:
